...
2. flush_memories() trims oversized conversations before the LLM call as
...
# ── Feasibility: flush model also checked ────────────────────────────────
...
class TestFeasibilityChecksFlushModel:
    ...
    def test_smaller_flush_model_lowers_effective_context(self, monkeypatch):
        ...
        # 80K is the effective aux; 80K - headroom < 100K → threshold lowered
    ...
    def test_same_model_overhead_still_triggers_correction(self, monkeypatch):
        ...
        threshold + system_prompt + tool_schema > aux_context.  The headroom
        deduction must fire even when aux_context >= threshold."""
        agent = _make_agent(monkeypatch)
        # 128K context, threshold just below context.
        # headroom = tool_overhead (~100) + 12K = ~12.1K
        # effective_limit = 128K - 12.1K = ~115.9K < 120K → triggers
